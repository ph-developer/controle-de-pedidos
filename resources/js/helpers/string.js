...
export function generateRandomString(length) {
    ...
    // Itera pela quantidade de vezes solicitadas (length) inserindo caracteres aleatórios na variavel inicial.
    for (let i = 0; i <= length; i++) {
        // Gera um número aleatório sendo no máximo igual ao tamanho da lista de caracteres permitidos.
        const randomNumber = Math.floor(Math.random() * chars.length);

        // Adiciona um caractere da lista de caracteres permitido, que está na posição referente ao número
        // aleatório gerado, à variável inicial.
